var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.col-span-1');tabs.forEach(function(it){var pz=pdfh(it,'.space-x-2:eq(0)&&Text');var ps='国内转播';var pk=pdfh(it,'.space-x-2:eq(1)&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.text-xs&&Text');var url=pd(it,'a:eq(3)&&href');items.push({desc:timer+'🏆'+ps,title:pz+'🆚'+pk,pic_url:img,url:url})});setResult(items);",
    二级:{
        ...
        "lists":"js:LISTS=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;let html=request(input);let data=pdfa(html,'.route-buttons');TABS.forEach(function(tab){let d=data.map(function(it){let name=pdfh(it,'.route-button&&Text');let url=pd(it,'.route-button&&data-url');return name+'$'+url});LISTS.push(d)});",
        },
    搜索:'',
}
