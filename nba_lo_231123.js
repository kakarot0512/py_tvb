var rule = {
    ...
    二级:{
        ...
        "lists":"js:LISTS=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;let html=request(input);let data=pdfa(html,'.route-buttons');TABS.forEach(function(tab){let d=data.map(function(it){let name=pdfh(it,'Text');let url=pd(it,'.route-buttons&&data-url');return name+'$'+url});LISTS.push(d)});",
        },
    搜索:'',
}
