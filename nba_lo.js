var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.col-span-1');tabs.forEach(function(it){var pz=pdfh(it,'.space-x-2:eq(0)&&Text');var ps='国内转播';var pk=pdfh(it,'.space-x-2:eq(1)&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.text-xs&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'🏆'+ps,title:pz+'🆚'+pk,pic_url:img,url:url:eq(1)})});setResult(items);",
    二级:{
